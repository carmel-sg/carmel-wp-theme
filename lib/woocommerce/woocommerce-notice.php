 ...
 * Carmel.
 *
 * This file adds the Genesis Connect for WooCommerce notice to the Carmel Theme.
 *
 * @package Carmel
...
add_action( 'admin_notices', 'carmel_woocommerce_theme_notice' );
/**
...
function carmel_woocommerce_theme_notice() {
	...
	if ( get_user_option( 'carmel_woocommerce_message_dismissed', get_current_user_id() ) ) {
		return;
	}

	/* translators: %s: child theme name */
	$notice_html = sprintf( __( 'Please install and activate <a href="https://wordpress.org/plugins/genesis-connect-woocommerce/" target="_blank">Genesis Connect for WooCommerce</a> to <strong>enable WooCommerce support for %s</strong>.', 'carmel' ), wp_get_theme()->get( 'Name' ) );
	...
	if ( current_user_can( 'install_plugins' ) ) {
		...
		$install_link = sprintf(
			...
			__( 'install and activate Genesis Connect for WooCommerce', 'carmel' )
		);

		/* translators: 1: plugin install prompt presented as link, 2: child theme name */
		$notice_html = sprintf( __( 'Please %1$s to <strong>enable WooCommerce support for %2$s</strong>.', 'carmel' ), $install_link, wp_get_theme()->get( 'Name' ) );
	}

	echo '<div class="notice notice-info is-dismissible carmel-woocommerce-notice"><p>' . wp_kses_post( $notice_html ) . '</p></div>';
...
function carmel_dismiss_woocommerce_notice() {
	...
	update_user_option( get_current_user_id(), 'carmel_woocommerce_message_dismissed', 1 );
...
function carmel_notice_script() {
	...
	wp_enqueue_script( 'carmel_notice_script', get_stylesheet_directory_uri() . '/lib/woocommerce/js/notice-update.js', [ 'jquery' ], '1.0', true );
...
function carmel_reset_woocommerce_notice() {
	...
	$args  = [
		'meta_key'   => $wpdb->prefix . 'carmel_woocommerce_message_dismissed', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
		'meta_value' => 1, // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_value
	];
	$users = get_users( $args );

	foreach ( $users as $user ) {
		delete_user_option( $user->ID, 'carmel_woocommerce_message_dismissed' );
	}
 ...
 */
function carmel_reset_woocommerce_notice_on_deactivation( $plugin, $network_deactivating ) {
	...
	carmel_reset_woocommerce_notice();
